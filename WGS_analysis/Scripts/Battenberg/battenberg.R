...
library(doParallel)
library(Battenberg)
setwd("/SAN/colcc/MMRd_HCEC_genomes/battenberg/")
listdata <- read.delim2(file="battenberg_sample_list.csv", header=TRUE, sep=",")
for (i in 1:nrow(listdata)){
  batchfolder <- listdata[i,1]
  TUMOURNAME <- listdata[i,2]
  NORMALNAME <- listdata[i,3]
  TUMOURBAM = paste("/SAN/colcc/MMRd_HCEC_genomes/bcbio_wgs/final/bam_files/",TUMOURNAME,"mkdub.bam",sep="")
  NORMALBAM = paste("/SAN/colcc/MMRd_HCEC_genomes/bcbio_wgs/final/bam_files/",NORMALNAME,"mkdub.bam",sep="")
  RUN_DIR = paste("/SAN/colcc/MMRd_HCEC_genomes/battenberg/")
  print(TUMOURBAM)
  print(NORMALBAM)
  print(RUN_DIR)
  if (listdata[i,4] == 'M'){
  ...
  analysis = "paired"
  JAVAJRE = "java"
  ALLELECOUNTER = "/share/apps/genomics/alleleCount-4.2.1/bin/alleleCounter"
  IMPUTE_EXE = "/share/apps/genomics/impute_v2.3.2_x86_64_static/impute2"
  BEAGLE_BASEDIR = "/SAN/colcc/MMRd_HCEC_genomes/battenberg/ref_files/"
  GENOMEBUILD = "hg38"
