# analyse Ensembl VEP results for SBS for all samples
...
# set the paths
path <- "/Volumes/NO NAME/ensembl_vep_results_denovo/SBS"
...
main_consequences <- c("downstream_gene_variant", "intergenic_variant", "synonymous_variant", "intron_variant", "missense_variant", "NMD_transcript_variant", "non_coding_transcript_variant")
...
impact_bar_chart <- ggplot(impact_counts_norm_df_long, aes(x = Genotype, y = Proportion, fill = Impact)) +
  ...
  labs(
    x = "Genotype",
    y = "Proportion of SBS with each impact type"
  )
...
pathogenic_list <- c("likely_pathogenic", "uncertain_significance,pathogenic,likely_pathogenic", "pathogenic", "pathogenic,likely_pathogenic", "likely_pathogenic,uncertain_significance")
