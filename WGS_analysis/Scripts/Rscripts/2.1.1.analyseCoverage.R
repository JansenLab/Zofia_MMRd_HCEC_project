...
coverageDir <- paste("/Users/zofiapiszka/Desktop/wgs_crispr_all_clones/coverage_files/", sep="")
#coverageDir <- arguments[2]
libType <- "WGS"

#make stats table
statsTab <- data.frame(matrix(NA, nrow = length(sampleList), ncol = 7)) # creates a matrix filled with NA values
# will have to change that to go by every second row 
names(statsTab) <- c("sample_id", "meanDepth", "meanCoverage", "duplicatedReads", "totalPercRem")
statsTab["sample_id"] <- sampleList
...
for(currSet in 1:nrow(statsTab)){
  ...
  plotNames <- statsTab[currSet, "sample_id"] # do I need this?
...
", round(metricsTemp[1, "MEAN_INSERT_SIZE"], digits = 2)), xpd=TRUE)
  ...
  alignmentTemp <- read.table(file = paste(coverageDir, "/", plotNames, "/", plotNames, ".alignment_metrics.txt", sep=""), sep="\t", header = TRUE, stringsAsFactors = FALSE) # edit 
