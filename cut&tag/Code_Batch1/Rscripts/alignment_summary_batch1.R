...
projPath = "/Users/zofiapiszka/Desktop/cut&tag/batch1/alignment_summary/"
sampleSheet <- read.csv("/Users/zofiapiszka/Desktop/cut&tag/batch1/batch1_sample_list.csv")
...
for(hist in sampleList){
  alignRes = read.table(paste0(projPath, hist, ".24Oct2024.txt"), header = FALSE, fill = TRUE)
  alignRate = substr(alignRes$V1[9], 1, nchar(as.character(alignRes$V1[9]))-1)
  histInfo = strsplit(hist, "_")[[1]]
  alignResult = data.frame(Genotype = histInfo[3], Histone = histInfo[4], Replicate = histInfo[5], 
                           SequencingDepth = alignRes$V1[4] %>% as.character %>% as.numeric, 
...
write.csv(alignResult, "/Users/zofiapiszka/Desktop/cut&tag/alignment_summary/alignment_stats.csv")
...
main_bowtie_plot <- ggarrange(fig3A, fig3B, fig3C, ncol = 3, common.legend = TRUE, legend="bottom")
print(main_bowtie_plot)

## Summarize the duplication information from the picard summary outputs.
projPath_dup <- "/Users/zofiapiszka/Desktop/cut&tag/picard_output/"

dupResult = c()
for(hist in sampleList){
  dupRes = read.table(paste0(projPath_dup, hist, "_picard.rmDup.txt"), header = TRUE, fill = TRUE)
